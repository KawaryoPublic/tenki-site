...
export default function EditShelfMapUI({ location, shelf, shelfEquipment, setShelfEquipment, size, height, add, setAdd, equipment }: { location: Location, shelf: Shelf, shelfEquipment: {equipment: EquipmentInstance, state: "none" | "added" | "updated" | "deleted"}[], setShelfEquipment: Dispatch<SetStateAction<{equipment: EquipmentInstance, state: "none" | "added" | "updated" | "deleted"}[]>>, size: number[], height: number, add: boolean, setAdd: Dispatch<SetStateAction<boolean>>, equipment: Equipment[] }) {
    ...
    return (
        ...
        >
            ...
            <button className={`fixed top-0 bottom-0 left-0 right-0 opacity-50 bg-black z-999 ${!add && "hidden"}`} onClick={() => setAdd(false)} ></button>
            <div className="w-full h-full border-2 relative">
                {
                    shelfEquipment.map(({ equipment, state }, i) => (
                        ...
                        <button 
                            ...
                            className={`absolute text-center text-xs md:text-sm overflow-hidden flex items-center justify-center hover:border-2 hover:z-2 ${equipment.id === -1 ? "bg-gray-200" : "bg-gray-300"} ${selectedIndex === i ? "border-2" : "border"}`}
                            style={{
                ...
                {
                    add &&
                    <WhiteFrameUI className={`z-1001 fixed left-3 right-3 md:left-[20%] md:right-[20%] shadow-2xl top-3`}>
                        <AddEquipmentForm
                            equipment={equipment}
                            shelf={shelf}
                            location={location}
                ...
                }
                {
                    selectedIndex !== -1 && 
                        ...
                        */
                       <WhiteFrameUI className={`z-1001 fixed left-3 right-3 md:left-[20%] md:right-[20%] shadow-2xl ${shelfEquipment[selectedIndex].equipment.position[1] + shelfEquipment[selectedIndex].equipment.size[1] < shelf.size[1] * 0.65 ? "top-3" : "bottom-3"}`}>
                            <EditEquipmentForm
