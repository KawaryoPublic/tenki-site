...
export default function DefaultInput({ title, name, type = "text", defaultValue, value, onChange, ref, className = "", label = false, required = false, min = undefined, max = undefined, accept = "image/*,application/pdf", defaultChecked = false }: { title: string, name: string, type?: string, defaultValue?: string | number, value?: string | number, onChange?: (e: ChangeEvent<HTMLInputElement>) => void, ref?: RefObject<HTMLInputElement> | ((element: HTMLInputElement) => void), className?: string, label?: boolean, required?: boolean, min?: number, max?: number, accept?: string, defaultChecked?: boolean }) {
    return (
