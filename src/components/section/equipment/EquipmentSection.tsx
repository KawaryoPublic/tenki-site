  ...
  return (
    <section className="flex-1 flex flex-col gap-3 w-full">
      <div className="flex justify-between items-center">
        ...
        <DefaultSearchForm title="検索(#をつけるとタグ)" className="w-[80%] md:w-[70%] lg:w-[50%]" defaultValue={`${title.join(" ")}${(title.length !== 0 && tags.length !== 0) ? " " : ""}${tags.map(tag => `#${tag}`).join(" ")}`} search={searchString => searchByTagsAndTitle("/equipment", searchString)} />
      </div>
      {
        loading ? <div className="flex-1 flex flex-col items-center font-bold text-xl">Loading...</div> :
        !equipment ? <div className="flex-1 flex flex-col items-center font-bold text-xl">機材一覧を読み込めませんでした</div> :
        equipment.length === 0 ? <div className="flex-1 flex flex-col items-center font-bold text-xl">機材はありません</div> : 
        <div className="flex flex-col gap-4">
          {
            equipment.map((equipment, index) => (
              <div key={index}>
                <EquipmentUI equipment={equipment} tier={tier} />
              </div>
            ))
          }
        </div>
