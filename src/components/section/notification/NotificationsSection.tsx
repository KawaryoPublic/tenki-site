  ...
  return (
    <section className="flex-1 flex flex-col gap-3 w-full">
      <div className="flex justify-between items-center">
        ...
        <DefaultSearchForm title="検索(#をつけるとタグ)" className="w-[30%]" defaultValue={`${title.join(" ")}${(title.length !== 0 && tags.length !== 0) ? " " : ""}${tags.map(tag => `#${tag}`).join(" ")}`} search={searchString => searchByTagsAndTitle("/notification", searchString)} />
      </div>
      {
